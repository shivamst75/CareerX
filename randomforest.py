...
def predict_stream(iq, tenth):
    # Predict the stream using Random Forest classifier
...
if predicted_stream in color_map:
    ax.scatter(user_iq, user_tenth, predicted_stream_value, color='black', marker='.', s=1000, label='User Input')
else:
...
ax.set_title('Random Forest Decision Boundary in 3D')
plt.legend()
plt.show()
